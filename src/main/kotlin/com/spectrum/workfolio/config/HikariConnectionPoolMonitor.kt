...
class HikariConnectionPoolMonitor(
     ...
     * PostgreSQL에서 직접 Connection을 종료하는 SQL 실행
     */
    ...
    fun evictIdleConnections() {
        if (dataSource is HikariDataSource) {
            try {
                ...
                // Idle Connection이 많거나 Pool이 거의 가득 찬 경우 강제 정리
                if (idle > 3 || total >= maxPoolSize * 0.8) {
                    logger.warn(
                        """
                        |🧹 오래된 Idle Connection 강제 정리 시작
                        |현재 상태: Idle=$idle, Total=$total, Max=$maxPoolSize
                    ...
                    try {
                        ...
                        // 5분 이상 idle 상태인 Connection 종료
                        val terminatedCount = jdbcTemplate.update(
                            """
                        ...
                        if (terminatedCount > 0) {
                            logger.info("✅ $terminatedCount개의 오래된 Idle Connection을 종료했습니다.")
                        } else {
                            logger.debug("종료할 Connection이 없습니다.")
                        }
                    } catch (e: Exception) {
                        logger.error(
