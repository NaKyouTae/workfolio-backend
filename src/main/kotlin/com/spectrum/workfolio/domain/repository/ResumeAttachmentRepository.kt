...
interface ResumeAttachmentRepository : JpaRepository<ResumeAttachment, String> {
    ...
    @Query("SELECT a FROM ResumeAttachment a WHERE a.resume.id = :resumeId AND a.isVisible = true ORDER BY a.priority ASC")
    fun findVisibleByResumeId(@Param("resumeId") resumeId: String): List<ResumeAttachment>

    fun findByResumeIdAndTypeOrderByPriorityAsc(resumeId: String, type: AttachmentType): List<ResumeAttachment>
}
